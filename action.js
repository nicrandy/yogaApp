...
var exerciseInfo = []; // load all exercies info
...
var basePoseInfoFolderLocation = "";
var workoutName = "";
var preloadedImages = [];

var playingAudio = false;// if audio is currently playing
var audioStarted = false; // if the audio has been started or not
var audioFile = 'audio/airtone.mp3'; // location of audio file to play    removed / from front of audio/airtone.mp3
var audioTimerFile = 'audio/timerSound.mp3' // location of audio file for timer       removed / from front of audio/timerSound.mp3
...
// set base pose folder location from URL
// set the loading screen to show workout selected
function setBasePoseInfoFolderLocation() {
    var url = window.location.search;
    workoutName = url.substring(url.indexOf("=") + 1);
    console.log(" workout name " + workoutName);
    basePoseInfoFolderLocation = "workouts/" + workoutName + "/";
    return workoutName;
}

var thisWorkoutFolderName = setBasePoseInfoFolderLocation();

// parse the workout info file related to workout images and their landmarks
function loadExerciseInfoJSON(callback) {
    var xobj = new XMLHttpRequest();
    xobj.overrideMimeType("application/json");
    xobj.open('GET', 'exerciseInfo.json', true); //removed / from front of exerciseInfo.json
...
loadExerciseInfoJSON(function (response) {
    // Do Something with the response e.g.
    exerciseInfo = JSON.parse(response); // save json info to global variable
    // let thisworkout = setBasePoseInfoFolderLocation();
    // loop exerciseInfo and find workout with same name as thisworkout
    for (let i = 0; i < exerciseInfo.length; i++) {
        console.log("random folder name = " + exerciseInfo[i].Folder_name);
        if (thisWorkoutFolderName == exerciseInfo[i].Folder_name) {
            // set the workout info to global variable
            exerciseInfo = exerciseInfo[i];

            console.log("random folder name = " + exerciseInfo.Folder_name);
            document.getElementById("status").innerHTML = "Loading " + exerciseInfo.Workout_name + " myogai workout";
...
var countDown = true; // boolean to track if the timer is counting down or counting up
var moveToNextPoseOnCountdownFinished = true; // go to next pose if coutdown reaches 0
// variables for coutdown instead of count up
var timeToHoldPose = 10000; // seconds to hold pose (x 1000)
var timeForThisPoseToHold = 0; // time to hold pose for this pose
function timeInPose() {
    if (!countDown) {
        ...
        if (currentScore > minScoreThreshold) {
            ...
            if (firstPass) {
                currentPoseStartTime = Date.now();
                firstPass = false;
                timeForThisPoseToHold = timeToHoldPose;
            }
            else {
                timeForThisPoseToHold -= Date.now() - currentPoseStartTime;
                currentPoseStartTime = Date.now();
            }
        }
        else {
        ...
        totalTime = timeForThisPoseToHold / 1000;
        totalTime = Math.round(totalTime * 10) / 10;
        let timeString = totalTime.toString();
        if (totalTime % 1 == 0) {
            timeString = totalTime + ".0";
        ...
        document.getElementById("time").innerText = "Time:  " + timeString;
        if (moveToNextPoseOnCountdownFinished) {
            if (timeForThisPoseToHold < 0) {
                // play the audio
                const timeOverSound = new Audio(audioTimerFile);
                timeOverSound.play();
...
function updateTimer() {
    ...
    // only play audio if not couting down timer
    if (!moveToNextPoseOnCountdownFinished) {
...
function updatePose() {
    if (currentPoseInThisWorkout > 1) {
        savedReportData(calculateRollingAverageAndHighScore()[0], totalTime)
    }
    ...
    if (countDown) {
        ...
        document.getElementById("time").innerText = "Time:  " + timeString;
        console.log("time is up, moving to next pose");
    }
    ...
    // save pose data if first pose finished
...
function drawSingleImage() {
    ...
    // fix this so it reads the best pose image from the file
    for (let i = 0; i < allYogaPoseInfo.length; i++) {
        if (allYogaPoseInfo[i].PoseNumber == currentPoseInThisWorkout) {
            let poseToDisplay = 'front'
...
function workoutFinished() {
    // save final scores and time to array for report
    savedReportData(calculateRollingAverageAndHighScore()[0], totalTime);
    ...
    for (let i = 0; i < allWorkoutData.length; i++) {
        console.log("workout data to be saved: " + allWorkoutData[i].currentScore + " time " + allWorkoutData[i].currentLandmarks);
    }
    // window.location.href = '/report/index.html?workoutData=' + JSON.stringify(savedReportDataArray) + '&workoutName=' + JSON.stringify(workoutName);
}

// save the average score and time per pose after each pose is completed
var savedReportDataArray = [];
function savedReportData(score, time) {
    score = parseInt(score) - 30;
    savedReportDataArray.push(score, time);
    console.log("savedReportDataArray: " + savedReportDataArray);
}
...
function dataToSave() {
    ...
    console.log("time: " + time + " currentPoseNumber: " + currentPoseNumber + " currentLandmarks: " + currentLandmarks);
    let workout = {
        "date": time,
